...
export class AudioSystem {
    ...
    private bgmSequence = [
        // Measure 1
        261.63, 0, 261.63, 311.13, 0, 392.00, 0, 311.13,
        // Measure 2
        349.23, 0, 349.23, 392.00, 0, 261.63, 0, 0,
        // Measure 3
        261.63, 0, 261.63, 311.13, 0, 392.00, 0, 311.13,
        // Measure 4
    ...
    private playTone(freqStart: number, freqEnd: number, duration: number, type: OscillatorType = 'sine') {
        if (!this.audioCtx) return;
        ...
        osc.frequency.exponentialRampToValueAtTime(Math.max(freqEnd, 1), this.audioCtx.currentTime + duration);
    ...
    private playMultipleTones(notes: { freqStart: number, freqEnd: number, duration: number }[], type: OscillatorType = 'sine') {
        if (!this.audioCtx) return;
        let timeOffset = 0;
        ...
        notes.forEach(note => {
            ...
            if (note.freqEnd !== note.freqStart) {
                osc.frequency.exponentialRampToValueAtTime(Math.max(note.freqEnd, 1), this.audioCtx!.currentTime + timeOffset + note.duration);
            }

            gainNode.gain.setValueAtTime(0.2, this.audioCtx!.currentTime + timeOffset);
            gainNode.gain.exponentialRampToValueAtTime(0.01, this.audioCtx!.currentTime + timeOffset + note.duration);
