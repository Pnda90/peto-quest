...
export class GameOverScene extends Phaser.Scene {
    ...
    create() {
        ...
        // Game Over Title
        this.add.text(w / 2, h / 3 - 50, 'GAME OVER', {
            fontSize: '64px',
            color: '#ff2222',
            fontStyle: 'bold'
        }).setOrigin(0.5);
        ...
        // Stats
        this.add.text(w / 2, h / 2 - 80, `Score: ${this.finalScore}`, {
            fontSize: '32px',
            color: '#ffffff'
        }).setOrigin(0.5);

        if (isNewHighScore) {
            const highscoreText = this.add.text(w / 2, h / 2 - 40, `NEW HIGH SCORE!`, {
                fontSize: '24px',
                color: '#ffcc00',
                fontStyle: 'bold'
            }).setOrigin(0.5);
        ...
        const rankIndex = leaderboard.findIndex(e => e.score === this.finalScore && e.distance === this.finalDistance);
        if (rankIndex >= 0 && rankIndex < 3) {
            const rankText = this.add.text(w / 2, 130, `#${rankIndex + 1} NELLA CLASSIFICA!`, {
                fontSize: '32px',
                color: '#ffcc00',
                fontStyle: 'bold',
                stroke: '#000',
        ...
        this.add.text(w / 2, h / 2, `Distance: ${this.finalDistance}m`, {
            fontSize: '24px',
            color: '#aaaaaa'
        }).setOrigin(0.5);

        this.add.text(w / 2, h / 2 + 40, `Beans Collected: +${this.finalCoins}`, {
            fontSize: '24px',
            color: '#ffcc00'
        }).setOrigin(0.5);
