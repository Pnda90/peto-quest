...
export class LeaderboardScene extends Phaser.Scene {
  ...
  create() {
    ...
    if (leaderboard.length === 0) {
      this.add
        .text(w / 2, h / 2, 'Nessun record ancora...\nInizia a scalare!', {
          fontSize: '24px',
          color: '#aaaaaa',
          align: 'center'
        })
      ...
      leaderboard.forEach((entry, index) => {
        ...
        this.add
          .text(w / 2 - 180, y, `${index + 1}.`, { fontSize: '24px', color })
          .setOrigin(0, 0.5);
        this.add
          .text(w / 2 - 120, y, `${entry.score}`, { fontSize: '24px', color, fontStyle: 'bold' })
          .setOrigin(0, 0.5);
        this.add
          .text(w / 2 + 50, y, `${entry.distance}m`, { fontSize: '20px', color: '#aaaaaa' })
          .setOrigin(0, 0.5);
        this.add
          .text(w - 50, y, entry.date, { fontSize: '18px', color: '#666666' })
          .setOrigin(1, 0.5);
      });
    }
