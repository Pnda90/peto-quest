...
export class MenuScene extends Phaser.Scene {
  ...
  create() {
    ...
    // Top Stats
    this.add.text(20, 20, `High Score: ${saveData.highScore}`, {
      fontSize: '24px',
      color: '#ffffff'
    });
    this.add
      .text(w - 20, 20, `Beans: ${saveData.coins}`, { fontSize: '24px', color: '#ffcc00' })
      .setOrigin(1, 0);

    // Title
    const title = this.add
      .text(w / 2, h / 3, 'PETO QUEST', {
        fontSize: '64px',
    ...
    if (activeMissions.length > 0) {
      ...
      this.add
        .text(w / 2, h - 215, 'MISSIONE ATTIVA', {
          fontSize: '14px',
          color: '#ffcc00',
          fontStyle: 'bold'
        })
        .setOrigin(0.5);
      this.add
        .text(w / 2, h - 195, `${mission.description}: ${progress}/${mission.goal}`, {
          fontSize: '16px',
          color: '#ffffff'
        })
        .setOrigin(0.5);
